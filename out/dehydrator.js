...
exports.semanticSkim = semanticSkim;
/**
 * 保护 Python 缩进的行压缩
 * 保留行首缩进，去除行尾空白，过滤空行
 */
function preservePythonIndentation(code) {
 ...
 * @param options 可选配置
 */
async function semanticSkim(code, options = {}) {
    ...
    const markdown = generateMarkdown(processed, savedPercent, variableMap, functionSignatures);
    return {
        markdown,
        originalLength,
        compressedLength,
 ...
 * 用签名替换函数体（优化隐藏文案）
 */
function replaceWithSignatures(code, signatures, threshold) {
    let result = code;
    for (const sig of signatures) {
        if (!sig.body || sig.body.trim().length === 0)
            continue;
 ...
 */
function shortenVariables(code, variableMap) {
    ...
    for (const { pattern, short } of commonPatterns) {
        ...
        if (!pattern.test(result))
            continue;
        // 检查长度是否确实缩短
        if (original.length <= short.length)
            continue;
        // 只记录实际发生的替换
 ...
 * 精简 Try-Catch
 */
function condenseTryCatch(code) {
    return code.replace(/try\s*\{[\s\S]*?\}\s*catch\s*\([^)]*\)\s*\{[\s\S]*?throw[^}]*;?\}?/gi, "// [Error handling hidden]");
}
/**
 * 逻辑锚点保留（核心新功能）
 * 保留：return、条件判断行、带 // ! 注释的行
 */
function preserveLogicAnchors(code) {
    const lines = code.split('\n');
 ...
 */
function generateMarkdown(content, savedPercent, variableMap, signatures) {
    const parts = [];
    // XML 标签包裹
